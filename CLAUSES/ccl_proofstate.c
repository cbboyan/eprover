...
ProofState_p ProofStateAlloc(FunctionProperties free_symb_prop)
{
   ...
   TBGCRegisterClauseSet(handle->terms, handle->frozen_store);
      ...
      bool print_skotypes, bool print_parents)
{
   ...
   if(print_skotypes)
   {
      fprintf(GlobalOut, COMCHAR" Training: Skolem types begin\n");
      SigPrintSkolemTypesTSTP(GlobalOut, state->signature);
      fprintf(GlobalOut, COMCHAR" Training: Skolem types end\n");
   }
...
void ProofStateStatisticsPrint(FILE* out, ProofState_p state)
{
   ...
   {
	   fprintf(out, COMCHAR" ...frozen by parental guidance       : %ld\n",
	  	   state->frozen_count);
	   fprintf(out, COMCHAR" ...of these subsequently unfrozen    : %ld\n",
