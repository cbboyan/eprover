...
1. `CLAUSES/ccl_tformulae.{c,h}` + `CLAUSES/ccl_formula_wrapper.c` — the
   check itself.
2. `TERMS/cte_typecheck.c` — four stray `assert(false)` that turn an ordinary
   type error into an abort in any debug build.
3. `CLAUSES/ccl_tformulae.c` — a second, independent segfault on ill-typed
   input found while testing this one: `p @ q` (applying a Boolean atom).

(2) and (3) are separable; happy to drop either.
...
## 2. `TERMS/cte_typecheck.c` — `assert(false)` before a graceful exit
...
(lines 327, 348, 358, 371); happy to drop this commit if you would rather
keep them.
...
## 3. `applied_tform_tstp_parse()` — a second segfault on ill-typed input

Independent of the above, found while probing the type checker. Applying
anything to a Boolean atom:
...
This is not caught by commit (1): it crashes *inside*
`TFormulaTSTPParse()`, before the new check ever runs.
...
none after. Before → after (only the changed rows):
...
139 -> 3   hq @ (^[X:$o]:(p & (f @ a))) (SIGSEGV, nested below a lambda)
 ...
 10 -> 10  (^[X:$o]:(X & p)) = (^[X:$o]:(p & X))
  3 ->  3  hq @ p                       (134 -> 3 in a debug build)
  4 ->  4  tff(c,axiom, f(a))
  3 ->  3  (f @ a @ a) = a
  3 ->  3  ((&) @ p @ q) @ q            (134 -> 3 in a debug build)
  3 ->  3  ! [F: $i > $i] : ((F @ p) = a)   (134 -> 3 in a debug build)
139 ->  3  p @ q                        (commit 3)
139 ->  3  (a = a) @ p                  (commit 3)
139 ->  3  $true @ p                    (commit 3)
```
...
the previous PR's corpus, and 105/105 real problem files. The remaining
no-footer files are pre-existing failures plus the three newly-rejected ones,
which exit via `Error()` like any other parse error.
